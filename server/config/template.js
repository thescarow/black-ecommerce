...
exports.merchantWelcome = (name) => {
  const message = {
    subject: "Merchant Registration",
    text:
      `${name}! Congratulations! Your application for merchant account has been accepted. \n\n` +
...
exports.signupEmail = (name) => {
  const message = {
    ...
    text: `${name.firstName} ${name.lastName}! Thank you for creating an account with us!.`,
    hasLink: false,
  };
...
exports.orderConfirmationEmail = (order) => {
  const message = {
    ...
    text:
      `${order.user.profile.firstName}! Thank you for your order!. \n\n` +
