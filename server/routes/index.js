...
router.post("/auth/register", async (req, res) => {
  try {
    ...
    await mail.sendEmail(registeredUser.email, "signup", null, registeredUser);
...
router.post("/auth/reset", auth, async (req, res) => {
  try {
    ...
    await mail.sendEmail(existingUser.email, "reset-confirmation");
...
router.post("/merchant/add", async (req, res) => {
  try {
    ...
    await mail.sendEmail(email, "merchant-application");
...
router.put("/merchant/:id/active", auth, async (req, res) => {
  try {
    ...
    if (!update.isActive) {
      ...
      await mail.sendEmail(merchantDoc.email, "merchant-deactivate-account");
    }
...
const createMerchantUser = async (email, name, merchant, host) => {
  ...
  if (existingUser) {
    ...
    await mail.sendEmail(email, "merchant-welcome", null, name);
    ...
    await mail.sendEmail(email, "merchant-signup", host, {
      resetToken,
      email,
    });
...
router.post("/newsletter/subscribe", async (req, res) => {
  ...
  await mail.sendEmail(email, "newsletter-subscription");
...
router.post("/order/add", auth, async (req, res) => {
  try {
    ...
    await mail.sendEmail(order.user.email, "order-confirmation", newOrder);
